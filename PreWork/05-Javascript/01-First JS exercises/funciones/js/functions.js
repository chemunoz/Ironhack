...
console.log( dniLetter1( 12345678 ) === 'Z');
console.log( dniLetter1( 34667892 ) === 'S');
console.log( dniLetter1( 92234488 ) === 'A');
...
function dniLetter2( dni ) {
	...
	else if (typeof(dni)=="number" && dni<0){
		result="Invalid parameter";
	}
	return result;
}
console.log( dniLetter2( -1 ) === 'Invalid parameter');
console.log( dniLetter2( "A1234567" ) === 'Invalid parameter');
...
function findLongestWord( array_words ){
	...
	for (x in array_words){
		if (array_words[x].length>max) {
			max=array_words[x].length;
		}
	}
	return max;
}
...
/*array_lengths.push( findLongestWord( ["Naruto", "Sasuke", "Sakura", "Hinata","Sharingan", "Kakashi", "Akamaru"] ) );*/
...
// isFinite()= heck whether a number is a finite, legal number.
...
function deleteRubbish( array )
{
	...
	for (x in array){
		if (array[x]!="*"){
			rubbish_array.push(array[x]);
		}
	}
	return rubbish_array;	
}
...
function arrangeElements( array )
{
	...
	var x;
	var number_array=[], letter_array=[];
	var result_array=[];
	
	for (x in array)
	{
		if (typeof(array[x])=="string"){
			letter_array.push(array[x]);
		}
		else if (typeof(array[x])=="number"){
			number_array.push(array[x]);
		}
	}
	
	/*Join both arrays*/
	for (x in number_array){
		result_array.push(number_array[x]);
	}
	for (x in letter_array){
		result_array.push(letter_array[x]);
	}
	return result_array;
}
...
function beautifyLetters( array )
{
	...
	for (x in array){
		switch (typeof(array[x])){
			case "string":
				if (array[x].toLowerCase()==="a" || array[x].toLowerCase()==="e" || array[x].toLowerCase()==="i" || array[x].toLowerCase()==="o" || array[x].toLowerCase()==="u"){
...
{
	//It receives an array with numbers and letters and returns it with its numbers beautified. 
